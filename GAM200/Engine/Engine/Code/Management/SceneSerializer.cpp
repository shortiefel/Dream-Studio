...
namespace Engine {
	...
	void SceneSerializer::Serialize(std::string filename) {
		FILE* fp;
		...
		for (size_t i = 0; i < num; i++) {
			...
			if (gCoordinator.HasCom<Camera2D>(cam, entList[i]) && cam != nullptr) {
				...
				objType.AddMember("Height", cam->height, doc.GetAllocator());
	...
	void SceneSerializer::Deserialize(std::string filename) {
		FILE* fp;
