...
namespace Engine
{
	...
	void TextureManager::Destroy()
	{
		...
		{
			glDeleteTextures(1, &a.second.handle);
		}

		LOG_INSTANCE("TextureManager destroyed");
	}

	GLuint TextureManager::LoadTexture(std::string filename, int* x, int* y, int* channels_in_files, int desired_channel, bool isTexture)
	{
		...
		if (textureList.find(filename) != textureList.end())
		{
			...
			*y = textureList[filename].height;
			...
			return textureList[filename].handle;
		}
		...
		textureList[filename].handle = texobj_hdl;
