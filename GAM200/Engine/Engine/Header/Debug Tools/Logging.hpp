...
#define SHOW_OVERLAP_COLLIDER 0
#define SHOW_MOUSE_OVERLAP_COLLIDER 0
...
namespace Engine {
	...
	public:
		...
		static bool CheckType(const Event& tem) {
			...
			if  constexpr (!SHOW_WINDOW_RESIZE) if (tem.GetEventType() == EventType::Window_Resize) return 1;
			if  constexpr (!SHOW_WINDOW_MOVED) if (tem.GetEventType() == EventType::Window_Moved) return 1;
			if  constexpr (!SHOW_KEY_PRESSED) if (tem.GetEventType() == EventType::Key_Pressed) return 1;
			if  constexpr (!SHOW_KEY_RELEASED) if (tem.GetEventType() == EventType::Key_Released) return 1;
			if  constexpr (!SHOW_MOUSE_MOVED) if (tem.GetEventType() == EventType::Mouse_Moved) return 1;
			if  constexpr (!SHOW_MOUSE_BUTTON_PRESSED) if (tem.GetEventType() == EventType::Mouse_Button_Pressed) return 1;
			if  constexpr (!SHOW_MOUSE_BUTTON_RELEASED) if (tem.GetEventType() == EventType::Mouse_Button_Released) return 1;
			if  constexpr (!SHOW_MOUSE_SCROLLED) if (tem.GetEventType() == EventType::Mouse_Scrolled) return 1;
			if  constexpr (!SHOW_OVERLAP_COLLIDER) if (tem.GetEventType() == EventType::Overlap_Collider) return 1;
			if  constexpr (!SHOW_MOUSE_OVERLAP_COLLIDER) if (tem.GetEventType() == EventType::Mouse_Overlap_Collider) return 1;
			if  constexpr (!SHOW_FIXED_UPDATE) if (tem.GetEventType() == EventType::Fixed_Update) return 1;
			return 0;
		}
