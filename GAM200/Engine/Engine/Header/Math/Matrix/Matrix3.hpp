...
namespace Engine {
	namespace DreamMath {
		namespace MathImplementation {
			...
			*************************************************************************/
			template <typename T>
			Vector3D<T>  operator * (const Matrix3<T>& pMtx, const Vector3D<T>& rhs) {
				Vector3D<T> result;
				result.x = (pMtx.m[0] * rhs.x) + (pMtx.m[3] * rhs.y) + (pMtx.m[6] * rhs.z);
				result.y = (pMtx.m[1] * rhs.x) + (pMtx.m[4] * rhs.y) + (pMtx.m[7] * rhs.z);
				result.z = (pMtx.m[2] * rhs.x) + (pMtx.m[5] * rhs.y) + (pMtx.m[8] * rhs.z);
				return result;
			}
